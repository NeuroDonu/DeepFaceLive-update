# C:\Users\neurodonu\Downloads\DeepFaceLive\DeepFaceLive\xlib\avecl\_internal\op\any_wise.py:
import numpy as np
...
def any_wise(op_text: str,
    ...
    """
    Элементарная операция с N входами.

    Аргументы:
        op_text     пример: O=(2*I0*I1)+I2
        *args       список [ Tensor | number ]
        dim_wise_axis(None)
        dtype       тип данных
        output_t    результат вычисления в этот тензор.
                    Тензор может иметь другую форму, но должен совпадать по общему размеру.
    """
    ...
    # --- CORRECTED LINE ---
    # HArgs.decompose likely returns 3 values: shapes, dtypes, kernel args
    decomposed_result = HArgs.decompose(args)
    if len(decomposed_result) == 4:
        shape_list, dtype_list, krn_args, _ = decomposed_result
    # Можно добавить print для отладки, если хотите:
        #print("DEBUG: HArgs.decompose returned 4 values")
    elif len(decomposed_result) == 3:
        shape_list, dtype_list, krn_args = decomposed_result
    # Можно добавить print для отладки, если хотите:
        #print("DEBUG: HArgs.decompose returned 3 values")
    else:
    # Если вернется что-то совсем неожиданное
        raise ValueError(f"HArgs.decompose вернул неожиданное количество значений: {len(decomposed_result)}")
    # --- END CORRECTION ---
    ...
    if output_t is None:
        ...
        raise ValueError(f'output_t должен иметь размер {op.o_shape.size}')
    ...
    def __init__(self, shape_list, dtype_list, dim_wise_axis, o_dtype, op_text: str):
        ...
        if dim_wise_axis is not None:
            ...
            if dim_wise_axis_size > 16:
                raise ValueError(f'размер dim_wise_axis > 16: {dim_wise_axis_size}')
